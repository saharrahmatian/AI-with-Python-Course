    ...
    if print_incorrect_dogs == True and n_correct_dogs + n_correct_notdogs != n_images:
        for key in results_dic:
            if sum(results_dic[key][3:]) == 1:
                print("Misclassified dog, The pet label is: {} and classifier label is :{}".format(results_dic[key][0], results_dic[key][1]))
                
    if print_incorrect_breed == True and n_correct_dogs != n_correct_breed:
        for key in results_dic:
            if sum(results_dic[key][3:]) == 2 and results_dic[key][2] == 0:
                print("Misclassified breed, The pet label is: {} and classifier label is :{}".format(results_dic[key][0],results_dic[key][1]))
    None
